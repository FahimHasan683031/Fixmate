...
// Initialize Firebase Admin SDK using credentials from configuration
const initializeFirebase = () => {
  try {
    if (admin.apps.length === 0) {
      ...
      if (!base64key) {
        logger.error(
          'Firebase initialization failed: FIREBASE_SERVICE_ACCOUNT_BASE64 not found in config',
        );
        return;
      }
...
// Send a push notification to a specific device token with optional data payload
